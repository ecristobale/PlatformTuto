...
public class EnemyHandler {
    ...
    private BufferedImage[][] sharkArray;
    ...
    private void drawSharks(Graphics g, int xLvlOffset) {
        for (Shark s : currentLevel.getSharks())
            if (s.isActive()) {
                g.drawImage(sharkArray[s.getState()][s.getAnimationIndex()], (int) s.getHitbox().x - xLvlOffset - SHARK_DRAWOFFSET_X + s.flipX(),
    ...
    private void drawPinkstars(Graphics g, int xLvlOffset) {
        for (Pinkstar p : currentLevel.getPinkstars())
            if (p.isActive()) {
                g.drawImage(pinkstarArray[p.getState()][p.getAnimationIndex()], (int) p.getHitbox().x - xLvlOffset - PINKSTAR_DRAWOFFSET_X + p.flipX(),
    ...
    public void checkEnemyHit(Rectangle2D.Float attackBox) {
        for(Crabby c : currentLevel.getCrabs())
            if (c.isActive())
                if (c.getState() != DEAD && c.getState() != HITTED)
        ...
        for (Pinkstar p : currentLevel.getPinkstars())
            if (p.isActive()) {
                if (p.getState() == ATTACK && p.getAnimationIndex() >= 3)
                    ...
                    if (p.getState() != DEAD && p.getState() != HITTED)
        ...
        for (Shark s : currentLevel.getSharks())
            if (s.isActive())
                if (s.getState() != DEAD && s.getState() != HITTED)
    ...
    private void loadEnemyImgs() {
        crabbyArray = getImgArray(LoadSave.getSpriteAtlas(LoadSave.CRABBY_SPRITE), 9, 5, CRABBY_WIDTH_DEFAULT, CRABBY_HEIGHT_DEFAULT);
        pinkstarArray = getImgArray(LoadSave.getSpriteAtlas(LoadSave.PINKSTAR_ATLAS), 8, 5, PINKSTAR_WIDTH_DEFAULT, PINKSTAR_HEIGHT_DEFAULT);
        sharkArray = getImgArray(LoadSave.getSpriteAtlas(LoadSave.SHARK_ATLAS), 8, 5, SHARK_WIDTH_DEFAULT, SHARK_HEIGHT_DEFAULT);
    }

    private BufferedImage[][] getImgArray(BufferedImage atlas, int xSize, int ySize, int spriteW, int spriteH) {
